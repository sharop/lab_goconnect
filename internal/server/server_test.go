...
import (
	...
	"github.com/sharop/lab_goconnect/internal/config"
...
func TestServer(t *testing.T) {
	for scenario, fn := range map[string]func(
		...
		t.Run(scenario, func(t *testing.T) {
			rootClient,
			nobodyClient,
			config, teardown := setupTest(t, nil)
			defer teardown()
...
func setupTest(t *testing.T, fn func(*Config)) (
	...
	if *debug {
		metricsLogFile, err := ioutil.TempFile("", "metrics-sharop-*.log")
		require.NoError(t, err)
		t.Logf("metrics log file: %s", metricsLogFile.Name())

		tracesLogFile, err := ioutil.TempFile("", "traces-sharop-*.log")
		require.NoError(t, err)
